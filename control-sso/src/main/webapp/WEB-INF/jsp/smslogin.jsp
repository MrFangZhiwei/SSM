...
<body>
	...
	var Idenfity = {
			...
			doLogin:function() {
				$.post("${pageContext.request.contextPath}/sms/idenfiy", $("#regUser").serialize(),function(data){
					if (data.status == 200) {
						//alert("获取验证码成功！");
						Idenfity.refresh();
					} else {
	...
	var LOGIN = {
			...
			doLogin:function() {
				$.post("${pageContext.request.contextPath}/sms/userlogin", $("#regUser").serialize(),function(data){
					if (data.status == 200) {
						alert("验证码通过！");
						if (redirectUrl == "") {
							//location.href = "/control/main";
							location.href = "http://show.keyonecn.com/manager/control/main";
						} else {
						  //location.href ="http://localhost:8080"+redirectUrl;
							location.href = "http://show.keyonecn.com"+redirectUrl;
						}
