...
public class SmsUserServiceImpl implements SmsUserService
{
	...
	@Value("${SSO_USER_SESSION_EXPIRE}")
	private Integer SSO_USER_SESSION_EXPIRE;
	
	@Value("${REDIS_USERCODE_SESSION_KEY}")
	private String REDIS_USERCODE_SESSION_KEY;
	...
	public KeyoneResult getidentifycode(String phone, HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (result.getStatus() == 200)
		{
			...
			//设置redis的过期时间为60秒
			jedisClient.expire(SSO_USER_SESSION_EXPIRE + ":" + token, SSO_USER_SESSION_EXPIRE);
			String code = result.getMsg();
			//将验证码存入redis中缓存15706844570
			jedisClient.set(REDIS_USERCODE_SESSION_KEY + ":" + token, code);
			//设置redis的过期时间为60秒
			jedisClient.expire(REDIS_USERCODE_SESSION_KEY + ":" + token, SSO_SMS_SESSION_EXPIRE);
			//添加写cookie的逻辑,cookie的有效期是关闭浏览器就失效  设置为6小时之后过期
			CookieUtils.setCookie(request, response, "SMS_TOKEN", token,21600);
			...
			HttpServletResponse response)
	{
		...
		//根据tokenredis中获取用户信息
		String code = jedisClient.get(REDIS_USERCODE_SESSION_KEY + ":" + token);
		//判断json是否空
		...
		} else
		{
			...
			{
				//设置验证码过期
				jedisClient.expire(REDIS_USERCODE_SESSION_KEY + ":" + token, -1);
	...
	public KeyoneResult getUserByToken(String token)
	{
		//根据tokenredis中获取用户信息
				...
				{
					return KeyoneResult.build(400, "此session已经过期,器重新登录");
				}
				//更新过期时间
				jedisClient.expire(REDIS_USER_SESSION_KEY + ":" + token, SSO_USER_SESSION_EXPIRE);
